...
# Introduction{-}
In this workshop, we consider principal component analysis which can be used to determine the directions of highest variation is a dataset. This is often used to reduce the dimension of the dataset by ignoring those dimensions where the variation is relatively small. The goal in this workshop is slightly different since we merely use the principal component analysis to estimate the variation in different directions. Two examples are given in @fig-examples.

The motivation for our analysis is that knowledge on these variations can be used to discover issues in wind turbines.^[See e.g. [https://orbit.dtu.dk/files/101029855/Eigen_PHM2014.pdf](https://orbit.dtu.dk/files/101029855/Eigen_PHM2014.pdf)] The idea is that the manufacturer of the wind turbine has published expected power outputs under certain conditions defined from variables such as wind speed, temperature, air density etc. Thus, we collect data about (some of) these variables along with the actual power output. Afterwards, we compare all of this data to the expected, theoretical power output under the given conditions.
If the wind turbine works optimally, we expect that the collected data points lie close to the theoretical power output, and that the variation around this will be relatively minor. If, on the other hand, the turbine has some issue (defective parts, ice on the wings or similar), we expect a greater variation in the collected data.

::: {#fig-examples layout-ncol=2}

![](fig1.svg){width=100% fig-alt="Plot with limited variation"}

![](fig2.svg){width=100% fig-alt="Plot with large variation"}

Two sets of observations with 2 variables and 100 observations each. In addition, the principal components (eigenvectors) are shown. The length of the components is determined from the size of the eigenvalues. The dataset on the left shows almost all variation in the direction of one component, whereas the dataset on the right is more equally varied in both directions.
:::

If we have collected $n$ data points in (column) vectors $\mathbf{x}_1,\mathbf{x}_2,\ldots,\mathbf{x}_n$ with $m$ variables, we represent these by an $n\times m$-matrix
$$X=\left[
    \begin{array}{rcl}
      -&\mathbf{x}_1^T&-\\
      -& \mathbf{x}_2^T &-\\
    ...
    \end{array}
  \right]$$
such that the $i$'th row contains the values of the $i$'th data point. In this workshop we assume that the data have *mean* $\mathbf{0}$, where the mean is defined as $\boldsymbol\mu=\frac{1}{n}(\mathbf{x}_1+\mathbf{x}_2+\cdots+\mathbf{x}_n)$.^[If this is not the case, we can instead use $\hat{\mathbf{x}}_i=\mathbf{x}_i-\boldsymbol\mu$ which *does* have mean $\mathbf{0}$]
The so-called *covariance matrix* $S$ for a data matrix $X$ with mean $\mathbf{0}$ is given by
$$S=\frac{1}{n-1}X^TX.$$
It turns out that each principal component is an eigenvector for $S$, and that the greater the corresponding eigenvector is, the greater the variation in that direction.

# Exercise 1{#sec-opg1}
Assume that we have collected the data points
$$\mathbf{x}_1=\left[
    \begin{array}{r}
      -3 \\ 0\\ 2
    \end{array}
...
1. Check that the data has mean $\mathbf{0}$.

1. Write out the matrix $X$ for the given data points. What is its rank, and what is the dimension of its null space?

1. Show that the covariance matrix of $X$ is
$$\frac{1}{3}\left[
      \begin{array}{rrr}
        30 & -15 & -20\\
        -15 & 14 & 12\\
...
# Exercise 2{#sec-opg2}
In this exercise, we consider a different covariance matrix
$$S=\left[
    \begin{array}{rrr}
      5 & 1 & 4\\
      1 & 5 & 4\\
    ...
    \end{array}
  \right],$$
  which has &lsquo;nicer&rsquo; eigenvalues and eigenvectors than the one in @sec-opg1. In @sec-opg4, you will get a data matrix $X$ which has $S$ as its covariance matrix.


1. Check that $\mathbf{v}_1=[1,1,2]^T$ is an eigenvector of $S$. What is the corresponding eigenvalue?

1. Show that the characteristic polynomial for $S$ is
$$-\lambda^3 + 20 \lambda^2 - 92 \lambda + 112.$${#eq-charPoly}

1. It may be shown that the polynomial in @eq-charPoly can be rewritten as $(\lambda-14)(-\lambda^2+6\lambda-8)$. Use this to determine the remaining eigenvalues of $S$.

1. Determine bases for the eigenspaces corresponding to the eigenvalues in question 3.

1. Use the above results to find a diagonalization of $S$.

# Exercise 3{#sec-opg3}
In this exercise, we explore why the eigenvector with the greatest eigenvalue describes the direction of highest variance.


(@) An $m\times m$ matrix $A$ is called symmetric if $A^T=A$. Show that a covariance matrix is always symmetric.

Later in the course, you will see^[It is the result called the *spectral theorem*] that for such a symmetric matrix $A$, it is possible to find a basis for $\mathbb{R}^m$ consisting of eigenvectors $\mathbf{v}_1,\mathbf{v}_2,\ldots,\mathbf{v}_m$ for $A$, such that
$$\left\{
    \begin{array}{l@{\hspace{1cm}}l}
      \mathbf{v}_i^T\mathbf{v}_i=1 & \text{for all } i\\
      \mathbf{v}_i^T\mathbf{v}_j=\mathbf{v}_j^T\mathbf{v}_i=0 & \text{for all pairs $(i,j)$, where $i\neq j$}
    \end{array}
  \right.$${#eq-basisProperties}

Assume in the exercises below that we have found such a basis from the covariance matrix $S$.


(@) Let $\mathbf{w}$ be a vector in $\mathbb{R}^m$, and let $X$ be a data matrix with covariance matrix $S$ like above. Define a new matrix $\tilde{X}=X\mathbf{w}$, and explain that $\tilde{X}$ is in $\mathrm{Col}(X)$.

(@) Show that the covariance matrix for $\tilde{X}$ is given by $\mathbf{w}^TS\mathbf{w}$. What is the size of this covariance matrix?
  
(@) Explain why there exist unique $c_1,c_2,\ldots,c_m$, such that
$$\mathbf{w}=c_1\mathbf{v}_1+c_2\mathbf{v}_2+\cdots+c_m\mathbf{v}_m.$$

(@) Combine&nbsp;3. og 4. with the properties in @eq-basisProperties to show that the covariance of $\tilde{X}$ is
$$c_1^2\lambda_1+c_2^2\lambda_2+\cdots+c_m^2\lambda_m.$${#eq-expandedCovariance}

(@) Assume that $\lambda_1$ is the greatest eigenvalue. Show that under the condition $c_1^2+c_2^2+\ldots+c_m^2=1$, @eq-expandedCovariance is maximized by setting $c_1=1$ and $c_i=0$ for $i\neq 1$.


# Delopgave 4{#sec-opg4}
Along with this workshop, you will find a Python file and two CSV-files: `plotPca.py`, `data1.csv` og `data2.csv`. The file `plotPca.py` contains a function that takes a data matrix as input, computes the principal components, and then plots them along with the data points.
The files `data1.csv` and `data2.csv` contain data matrices, on which you will perform principal component analysis.

::: {.callout-note}
The Python code works best if run locally. Then the plots will be interactive, such that they can be rotated and viewed from different angles.

You can also run them directly from this file, but in that case the plots are simply figures.
:::

If running the script locally, you can read in the two CSV-files directly. To run it in the browser, we can &lsquo;cheat&rsquo; by storing them as `StringIO` objects. In that way, they act like files even though they are not stored on a drive.
...
1. Use `numpy.loadtxt("data1.csv", delimiter=",")` to load in the first data matrix. You can use the code block at the bottom of this document. How many variables and observations does the data set have? til at indlæse den første datamatrix.

1. Perform the principal component analysis on `data1` by using the function `plotPca`. Do the components seem to describe variation as expected?

1. Now, perform the same analysis on the data found in `data2.csv`.

1. If we imagine that the collected data were wind turbine data, and that we &ndash; as in the introduction &ndash; expect that most of the variation should be found in one direction, which turbine would be the first the check for issues?

```{pyodide-python}
try: data1 # Only read in data1Fil, if data1 does not exist
...
::: {.callout-warning}
Remember to save your Python code in a separate file. If you close this window and open the HTML-file again, it will show the original code without your modifications.
:::
